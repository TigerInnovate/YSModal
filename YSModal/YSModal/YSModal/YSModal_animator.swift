...
extension YSModal_animator:UIViewControllerAnimatedTransitioning,CAAnimationDelegate{
    ...
    private func presentedDirection() -> YSModal_direction{
        return presentedVC?.ys_setupDirectionAndLength().direction ?? .toTop
    }
    ...
    private func presentedLength() -> CGFloat{
        ...
        if direction == .toTop || direction == .toBottom{ // 上下弹框
            var length = presentedVC?.ys_setupDirectionAndLength().length ?? sh * 0.5
            if length < 0{
                length = 0
            } else if length > sh{
                length = sh
            ...
            var length = presentedVC?.ys_setupDirectionAndLength().length ?? UIScreen.main.bounds.width * 0.5
            if length < 0{
                length = 0
            } else if length > sw{
                length = sw
